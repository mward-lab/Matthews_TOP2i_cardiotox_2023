...
```{r intial analysis, eval=TRUE, echo=FALSE,  fig.width=10, message=FALSE, warning=FALSE}
...
```{r DA24, eval=TRUE, echo=FALSE,  message=FALSE, warning=FALSE}
...
total24n <-list(sigVDA24$ENTREZID,sigVDX24$ENTREZID,sigVEP24$ENTREZID,sigVMT24$ENTREZID)
total24s <- list(sigVDA24$SYMBOL,sigVDX24$SYMBOL,sigVEP24$SYMBOL,sigVMT24$SYMBOL)
total3 <- list(sigVDA3$SYMBOL,sigVDX3$SYMBOL,sigVEP3$SYMBOL,sigVMT3$SYMBOL)
venn24n <- ggVennDiagram(total24n,
  ...
  ggtitle("24 hour gene list using ENTREZID")+
   theme(plot.title = element_text(hjust = 0.5))

venn24n
venn24s <- ggVennDiagram(total24s,
              category.names = c("Daunorubicin","Doxorubicin","Epirubicin","Mitoxantrone"),
              show_intersect = FALSE,
              set_color = "black",
              label = "both",
              ...
              edge_lty = "solid")+
  scale_x_continuous(expand = expansion(mult = .2))+
  ggtitle("24 hour gene list using SYMBOL")+
   theme(plot.title = element_text(hjust = 0.5))
venn24s
```
