...
```{r GO DA24 genelist of SDEGs}


gostsigvda24 <- gost(query =sigVDA24$ENTREZID,  
                      organism = "hsapiens",
                      ordered_query = FALSE,
                      domain_scope = "custom",
                      measure_underrepresentation = FALSE,
                      ...
                      custom_bg = backGL$ENTREZID,
                      sources=c("GO:BP"))
gosigvda24 <- gostplot(gostsigvda24, capped = FALSE, interactive = TRUE)
gosigvda24
# (gostres$result$p_value)
tablesigVDA24 <- gostsigvda24$result %>%
  dplyr::select(c(source, term_id, term_name,intersection_size, term_size, p_value)) #%>% 
  # mutate_at(.cols = 6, .funs= scientific_format()) %>% 
  # kable(.,) %>% 
  # kable_paper("striped", full_width = FALSE) %>%  
  ...
  tablesigVDA24 %>% 
   ...
   ggplot(., aes(x = log_val, y =reorder(term_name,p_value))) +
    ...
    ggtitle('sigDEG-DA24 GO:BP terms') +
    xlab("-log 10 (p-value)")+
    ylab("GO: BP term")+
      theme_bw()
...
```{r GO DX genelist of SDEGs}
gostsigvdx24 <- gost(query =sigVDX24$ENTREZID,  
                      organism = "hsapiens",
                      ordered_query = FALSE,
                      domain_scope = "custom",
                      measure_underrepresentation = FALSE,
                      ...
                      custom_bg = backGL$ENTREZID,
                      sources=c("GO:BP"))
gosigvdx24 <- gostplot(gostsigvdx24, capped = FALSE, interactive = TRUE)
gosigvdx24

tablesigVDX24 <- gostsigvdx24$result %>%
  dplyr::select(c(source, term_id, term_name,intersection_size, term_size, p_value)) 
  tablesigVDX24
  ...
  tablesigVDX24 %>% 
   ...
   ggplot(., aes(x = log_val, y =reorder(term_name,p_value))) +
    geom_point(aes(size = intersection_size)) +
    ggtitle('sigDEG-DX24 GO:BP terms') +
    xlab("-log 10 (p-value)")+
    ylab("GO: BP term")+
      theme_bw()
...
```{r GO EP genelist of SDEGs}

gostsigvEP24 <- gost(query =sigVEP24$ENTREZID,  
                      organism = "hsapiens",
                      ordered_query = FALSE,
                      domain_scope = "custom",
                      measure_underrepresentation = FALSE,
                      ...
                      custom_bg = backGL$ENTREZID,
                      sources=c("GO:BP"))
gosigvEP24 <- gostplot(gostsigvEP24, capped = FALSE, interactive = TRUE)
gosigvEP24

tablesigVEP24 <- gostsigvEP24$result %>%
  dplyr::select(c(source, term_id, term_name,intersection_size, term_size, p_value)) 
  ...
  tablesigVEP24 %>% 
   ...
   ggplot(., aes(x = log_val, y =reorder(term_name,p_value))) +
    geom_point(aes(size = intersection_size)) +
    ggtitle('sigDEG-EP24 GO:BP terms') +
    xlab("-log 10 (p-value)")+
    ylab("GO: BP term")+
      theme_bw()
...
```{r GO MT genelist of SDEGs}

gostsigvMT24 <- gost(query =sigVMT24$ENTREZID,  
                      organism = "hsapiens",
                      ordered_query = FALSE,
                      domain_scope = "custom",
                      measure_underrepresentation = FALSE,
                      ...
                      custom_bg = backGL$ENTREZID,
                      sources=c("GO:BP"))
gosigvMT24 <- gostplot(gostsigvMT24, capped = FALSE, interactive = TRUE)
gosigvMT24

tablesigVMT24 <- gostsigvMT24$result %>%
  dplyr::select(c(source, term_id, term_name,intersection_size, term_size, p_value)) 
  ...
  tablesigVMT24 %>% 
   ...
   ggplot(., aes(x = log_val, y =reorder(term_name,p_value))) +
    geom_point(aes(size = intersection_size)) +
    ggtitle('sigDEG-MT24 GO:BP terms') +
    xlab("-log 10 (p-value)")+
    ylab("GO: BP term")+
      theme_bw()
...
```{r GODDEMresp genelist of SDEGs}
DDEMresp <- list24totvenn$..values..[[1]]
gostDDEMresp <- gost(query =DDEMresp,  
                      organism = "hsapiens",
                      ordered_query = FALSE,
                      domain_scope = "custom",
                      measure_underrepresentation = FALSE,
                      ...
                      custom_bg = backGL$ENTREZID,
                      sources=c("GO:BP"))
goDDEMresp <- gostplot(gostDDEMresp, capped = FALSE, interactive = TRUE)
goDDEMresp

tableDDEMresp <- gostDDEMresp$result %>%
  dplyr::select(c(source, term_id, term_name,intersection_size, term_size, p_value)) 
  ...
  tableDDEMresp %>% 
   ...
   ggplot(., aes(x = log_val, y =reorder(term_name,p_value))) +
    geom_point(aes(size = intersection_size)) +
    ggtitle('DDEMresp GO:BP terms') +
    xlab("-log 10 (p-value)")+
    ylab("GO: BP term")+
      theme_bw()
DDEMlist <-  list(DDEMresp,TI_respset$TI_respset,
                       ER_respset$ER_respset, 
                       LR_respset$LR_respset)     
ggVennDiagram(DDEMlist,
              category.names = c("DDEM","TI","ER","LR"),
              show_intersect = FALSE,
              set_color = "black",
              label = "both",
              label_percent_digit = 1,
...
scale_fill_distiller(palette="Greens")+
  labs(title = "24 hour comparison of significant DE genes", caption = " genes")+
  theme(plot.title = element_text(size = rel(1.5), hjust = 0.5))
...
```{r DDEresp  }
DDEresp <- list24totvenn$..values..[[9]]
gostDDEresp <- gost(query =DDEresp,  
                      organism = "hsapiens",
                      ordered_query = FALSE,
                      domain_scope = "custom",
                      measure_underrepresentation = FALSE,
                      ...
                      custom_bg = backGL$ENTREZID,
                      sources=c("GO:BP"))
goDDEresp <- gostplot(gostDDEresp, capped = FALSE, interactive = TRUE)
goDDEresp

tableDDEresp <- gostDDEresp$result %>%
  dplyr::select(c(source, term_id, term_name,intersection_size, term_size, p_value)) 
  ...
  tableDDEresp %>% 
   ...
   ggplot(., aes(x = log_val, y =reorder(term_name,p_value))) +
    geom_point(aes(size = intersection_size)) +
    ggtitle('DDEresp GO:BP terms') +
    xlab("-log 10 (p-value)")+
    ylab("GO: BP term")+
      theme_bw()
DDElist <-  list(DDEresp,TI_respset$TI_respset,
                       ER_respset$ER_respset, 
                       LR_respset$LR_respset)     
ggVennDiagram(DDElist,
              category.names = c("DDE","TI","ER","LR"),
              show_intersect = FALSE,
              set_color = "black",
              label = "both",
              label_percent_digit = 1,
...
```{r no response cluster}

NRresp <- read_csv("data/cormotif_NRset.txt")
gostrescoNR <- gost(query = c(NRresp),  
                      organism = "hsapiens",
                      ordered_query = FALSE,
                      domain_scope = "custom",
                      measure_underrepresentation = FALSE,
...
# (gostres$result$p_value)
tabletop2Biresp <- gostresTop2bi$result %>%
  dplyr::select(c(source, term_id, term_name,intersection_size, term_size, p_value)) #%>% 
  # mutate_at(.cols = 6, .funs= scientific_format()) %>% 
  # kable(.,) %>% 
  # kable_paper("striped", full_width = FALSE) %>%  
...
tabletop2Biresp
  ...
  tabletop2Biresp %>% 
   ...
   ggplot(., aes(x = log_val, y =reorder(term_name,p_value))) +
    geom_point(aes(size = intersection_size)) +
    ggtitle('24 hour response cluster GO:BP terms') +
    xlab("-log 10 (p-value)")+
    ylab("GO: BP term")+
      theme_bw()
