...
volcanosig <- function(df, psig.lvl, fcsig.lvl,topg) {
    df <- df %>% 
    mutate(threshold = ifelse(-log(adj.P.Val) >= psig.lvl & logFC >= fcsig.lvl,"A",
      ifelse(-log(adj.P.Val) >= psig.lvl & logFC <= -fcsig.lvl,"B", "C")))
    
    df <- df %>% mutate(genelabels = "")
    df$genelabels[1:topg] <- df$SYMBOL[1:topg]
  ggplot(df, aes(x=logFC, y=-log(adj.P.Val))) + 
    geom_point(aes(color=threshold))+
    geom_text_repel(aes(label = genelabels), segment.curvature = -1e-20,
    arrow = arrow(length = unit(0.015, "npc")), max.overlaps = Inf) +
    geom_hline(yintercept = -log(psig.lvl))+
...
volcanosig(V.DA.top, 0.01,1,15)+ ggtitle("Daunorubicin 3 hour")
volcanosig(V.DA24.top, 0.01,1,15)+ ggtitle("Daunorubicin 24 hour")
volcanosig(V.DX.top, 0.01,1,15)+ ggtitle("Doxorubicin 3 hour")
volcanosig(V.DX24.top, 0.01,1,15)+ ggtitle("Doxorubicin 24 hour")
volcanosig(V.EP.top, 0.01,1,15)+ ggtitle("Epirubicin 3 hour")
volcanosig(V.EP24.top, 0.01,1,15)+ ggtitle("Epirubicin 24 hour")
volcanosig(V.MT.top, 0.01,1,15)+ ggtitle("Mitoxatrone 3 hour")
volcanosig(V.MT24.top, 0.01,1,15)+ ggtitle("Mitoxatrone 24 hour")
volcanosig(V.TR.top, 0.01,1,15)+ ggtitle("Trastuzumab 3 hour")
volcanosig(V.TR24.top, 0.01,1,15)+ ggtitle("Trastuzumab 24 hour")
