...
library(Cormotif)
library(pheatmap)
# barcodeplot(efit2$t[,6],sigVDA24$ENTREZID,   labels = c ("Down in Daun 24", "up in Daun 24"))
annocor <- anno$group
colnames(countstotal) <- label

compmat <- matrix(group,nrow=6, ncol =12,byrow=TRUE)

shortcompmat <- matrix(rep(c(1,0,0,1), 3),nrow = 12, ncol=2, byrow=TRUE)


fittingcorr12 <- cormotiffit(countstotal,compmat, annocor, K=12)
fittingcormot <- cormotiffit(countstotal,compmat, anno$group, K=5)

plotMotif(fittingcorr12, title= "first 72 Motif attempt")
plotMotif(fittingcormot, title = "5 clusters")
...
pheatmap::pheatmap(mcor, cluster_rows = FALSE, cluster_cols = TRUE)

my_hclust<- hclust(dist(mcor), method = "complete")
...
my_heatmap <- pheatmap(mcor, silent = TRUE)
