...
Hello!   I am first loading all the beautiful libraryies I will need.
...
Now to load in the data.  Note:  this first bit uses files from my actual computer.  I will change this in the future.
...
###filtering here would leave 24931 genes and remove 3464,  That is too many!

###if I use the filter by expression
keep <- filterByExpr.DGEList(mymatrix, group = group)
filter_test <- mymatrix[keep, , keep.lib.sizes=FALSE]
dim(filter_test)
##this gives me 14448 genes left using a minimum of 10 read counts in each set
# minimum number of samples (I think this is 2)

##another way to filter using subset
filcpm_matrix <- subset(lcpm, (rowMeans(lcpm)>0))
dim(lcpm)
dim(filcpm_matrix)

####another way to do it but this keeps the matrix DGElist set up
...
```{r testing if they are the same}
###
hist(cpm(filt_matrix, log = TRUE))
hist(cpm(filter_test$counts, log = TRUE))
hist(cpm(mymatrix, log = TRUE))
##to test which genes are not in one or the other set)
apples <- rownames(filcpm_matrix)
oranges <- filt_matrix$genes

match(oranges$ENTREZID,apples)
not_matching_indices <- which(!(apples %in% filt_matrix$genes))

print(apples[not_matching_indices])

print(oranges[setdiff(oranges$ENTREZID, filter_test$genes$ENTREZID)])
