...
GTEXcr8z <- GTExsum %>% 
  ...
  pivot_longer(cols=n:total, names_to="group",values_to="total") %>% mutate(group=case_match(group, "n"~"In set","total"~"Not in set",.default = group))  %>%
  mutate(group=factor(group,levels = c("Not in set" ,"In set"))) %>% 
  mutate(Label=if_else(set=="nQTL"&group=="In set", paste0("*"),NA)) 
##add significance stars to graph using sig-dataframe
# GTEX_sig24 <- readRDS("data/GTEX_sig24.RDS")
# data_text <- data.frame(Label=c("ns","ns","*","*"),id=c("Daunorubcin","DOX","EPI","MTX"))

GTEXcr8z %>% 
  mutate(id= factor(id, levels =c("DOX","DNR","EPI","MTX", "TRX", "VEH"))) %>% 
...
Knowles_count <- 
  ...
  mutate(group=case_match(group, "y"~"In set","n"~"Not in set",.default = group))  %>%
...
```{r DOXreQTLs across other treatments,fig.height=3}
...
DOXeQTL_table %>%
  ...
  mutate(id=factor(id, levels=c("DNR","EPI", "MTX","Dox-specific\n DEGs"))) %>% 
ggplot(., aes(y=id,x=count,fill=group))+
  ...
  xlab("proportion")+
   scale_color_manual(values=c("red4","navy"))+
    scale_fill_manual(values=c("#2297E6","red2"), labels=c("not DOXreQTL","DOXreQTL"))+
  ...
  df <- cpmcounts
    df_plot <- df %>% 
      ...
      mutate(drug =case_match(drug, "Da"~"DNR",
                            ...
                            "Tr"~"TRZ",
                            "Ve"~"VEH", .default = drug)) %>% 
       mutate(drug=factor(drug, levels = c('DOX','DNR','EPI','MTX','TRZ','VEH')))
...
cpm_boxplot(cpmcounts,GOI='57338',"Dark2",drug_pal_fact,
