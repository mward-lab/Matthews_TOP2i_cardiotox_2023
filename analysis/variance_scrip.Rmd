...
knitr::opts_chunk$set(dev = c('svglite','png'),
	echo = FALSE,
	message = FALSE,
	warning = FALSE
)
...
statfun24 %>% 
  full_join(., statfun3, by="ENTREZID") %>% 
   ...
   # rowwise() %>% 
   cor(.) %>% 
   Heatmap(., column_title = "3 & 24 hr F stat corr", col=col_fun)
...
framefun24 %>% 
  ...
  column_to_rownames('ENTREZID') %>% 
   ...
   # rowwise() %>% 
   cor(.) %>% 
   Heatmap(., column_title = "24 and 3 hour pval corr", col=col_fun)

cor(x=matstat24,y=matframe24, use="pairwise") %>% 
  Heatmap(., column_title = "Fstat and pvalue rows", col=col_fun,clustering_distance_rows = "pearson")

cor(x=matstatall,y=matframeall, use="pairwise") %>% 
  Heatmap(., column_title = "Fstat and pvalue cols", col=col_fun,clustering_distance_columns = "pearson")

cor(x=matstatall,y=matframeall) %>% 
  Heatmap(., column_title = "Fstat and pvalue euc", col=col_fun)
...
qvalue3hr %>%  
  ...
  select(!TRZqvalues) %>% 
   ...
   Heatmap(., column_title = "3 hour q value corr", col=col_fun)
...
qvalue24hr %>%  
  ...
  select(!MTXqvalues) %>%
   ...
   Heatmap(., column_title = "24 hour q value corr", col=col_fun)
...
statfun24alt <- data.frame(ENTREZID=rownames(organized_drugframe))
for (i in 1:length(set24)){
  a <- set24[i]
    s_list<- Var_test_list24alt[[i]][[13]]
  names(s_list) <- rownames(organized_drugframe)
  hold<- map_df(s_list, ~as.data.frame(.x$statistic), .id="ENTREZID")
   statfun24alt[paste0(a)] <- hold$`.x$statistic`
 
}
statfun24alt %>% 
  column_to_rownames("ENTREZID") %>% 
  cor(.) %>% 
Heatmap(., column_title = "24 hour alt = greater hypothesis F stat corr", col=col_fun)
