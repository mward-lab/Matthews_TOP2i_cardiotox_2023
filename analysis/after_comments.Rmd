...
LD50tox_table<- readRDS("output/LD50tox_table.RDS")
LD50tox_table %>% 
  # mutate(LD50=log(LD50)) %>% 
ggplot(., aes(x=LD50, y=cpm))+
  geom_point(aes(color=indv))+
  geom_smooth(method="lm")+
  # stat_regline_equation(label.x.npc = "center")+
  facet_wrap(~drug, scales = "free_x")+
  labs(title="RARG", y = "delta cpm", x = "LD50")+
  scale_color_brewer(palette = "Dark2",guide="legend",name = "Individual",labels(c(1,2,3,4,5,6)))+
  theme_bw()+
  ggpubr:: stat_cor(method="pearson",
                       # cor.coef.name="rho",
...
DOXreGene <- readRDS("output/DOXreQTLs.RDS")
