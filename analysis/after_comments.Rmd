...
Top2_expression %>% 
  pivot_longer(cols=!c(Name,Description), names_to = "indv", values_to = "counts") %>% 
  filter(Description=="TOP2A") %>% 
  summary()
Top2_expression %>% 
  pivot_longer(cols=!c(Name,Description), names_to = "indv", values_to = "counts") %>% 
  filter(Description=="TOP2B") %>% 
  summary()
...
# schneider_closest_output <- read_delim("C:/Users/renee/AppData/Roaming/MobaXterm/slash/RemoteFiles/2687104_3_27/output.bed",delim = "\t", escape_double = FALSE,col_names = c("chrom","start","stop","RSID","Pvalue","chrom_gene","start_gene","end_gene","transcript"), trim_ws = TRUE)
...
just_the_SNPs <- SNP_bedtools_data %>% 
  distinct(.,ensembl_gene_id,.keep_all = TRUE)
...
new_Gwas_genes_sig <- FC_mine %>% 
  dplyr::filter(ENTREZID %in% list_snp_IDS$entrezgene_id) %>% 
...
new_gwas_mat <-  FC_mine %>% 
  ...
  filter(ENTREZID %in% list_snp_IDS$entrezgene_id) %>% 
...
ComplexHeatmap::Heatmap(new_gwas_mat, name = "Fold change\nvalues", 
        ...
        column_title = "Fold change values of GWAS `new set` genes", 
...
# simpnames <- data.frame(SYMBOL=c("PRDX6","PRODH","CDPF1" ,"RSU1", "ESD", "PPARA" ,"PRR5L", "WDHD1",  "PAG1",
#           "ADAMTSL1", "DENND3","PLEKHA5","NEDD4L","TRIB2","KTN1","GAREM1", "CLYBL",
#           "MSI1","DUBR", "NFE2L3","CALD1","PAM" ,"SIRT4","LRRTM4","ADCY2" ,"SULF1"))

# simpnames %>% 
#   left_join(., (SNP_gene_list %>% select(SYMBOL,Pvalue))) %>% 
#   arrange(Pvalue) %>% 
#   distinct(SYMBOL) %>% 
#   print
```
...
Heatmap(logFtestSNP_mat, name = "-Log10 \n p value", 
        ...
        # col = col_funFC,
        row_order = c('DOX','EPI','DNR', 'MTX', 'TRZ'),
        column_title = "-log10 Ftestpvalue SNP genes", 
        column_title_side = "top",
        column_order= c("LRRTM4","CLYBL",	"ADCY2","NEDD4L",	"PLEKHA5"	,	"PAG1",	"PPARA","PRODH", "CALD1","PAM",  "CDPF1",	"DUBR","PRR5L","RSU1",	"WDHD1"	,	"PRDX6","MSI1",	"SIRT4","GAREM1",	"ADAMTSL1","DENND3"	, "ESD"	,	"TRIB2"	,"KTN1","NFE2L3",	"SULF1"), 
        column_title_gp = gpar(fontsize = 16, fontface = "bold"),
        column_names_rot = 90, 
...
RSID_QTL_list_full <- read.csv("output/RSID_QTL_list_full.txt", row.names = 1)
...
new_egenes_mat <-  FC_mine %>% 
  filter(time=="24_hours") %>% 
  mutate(logFC= logFC*(-1)) %>%
  filter(ENTREZID %in% egene_SNP$entrezgene_id) %>% 
  dplyr::select(SYMBOL ,time, id, logFC) %>% 
  pivot_wider(id_cols=id, 
              names_from = SYMBOL, 
              values_from = logFC) %>% 
...
ComplexHeatmap::Heatmap(new_egenes_mat, name = "Fold change\nvalues", 
        ...
        # col = col_funFC,
        row_order = c('DOX','EPI','DNR', 'MTX','TRZ'),
        column_title = "Fold change values new GWAS eGenes", 
        column_title_side = "top",
        column_title_gp = gpar(fontsize = 16, fontface = "bold"),
        # column_order= c('RARG',
        #                 'TNS2', 
...
ComplexHeatmap::Heatmap(new_egenes_mat, name = "Fold change\nvalues", 
        ...
        # col = col_funFC,
        row_order = c('DOX','EPI','DNR', 'MTX','TRZ'),
        column_title = "Log10 pvalue GWAS eGenes", 
        column_title_side = "top",
        column_title_gp = gpar(fontsize = 16, fontface = "bold"),
...
```
The eGenes from gTEX are in order of adj. P.Value found in the Fold Change.  I can order another way if wanted.


```{r making Schneider_completeSNP_egenes,  include=TRUE,eval=FALSE}

list_snp_IDS <- readRDS("output/SNP_list_ID.RDS")##list of all SNP near genes with ensemble/hgnc/entrezid ID
RSID_QTL_list_full <- read.csv("output/RSID_QTL_list_full.txt", row.names = 1)##closest annotated gene to complete rsid list
SNP_supp <- readRDS("output/SNP_supp.RDS")## has the pvalue (this is the supplement from that paper)
egene_of_SNP <- SNP_egenes_allfiles %>% 
  distinct(ensembl_gene_id,.keep_all = TRUE) %>% 
  left_join(., (SNP_supp %>% dplyr::select(RSID,Pvalue)),by = join_by(RSID)) %>% dplyr::select(SYMBOL, ensembl_gene_id, entrezgene_id,Pvalue)
# contains the eGenes to every SNP
ESGN_rds <- readRDS("output/ESGN_rds.RDS")


SNP_genes <- RSID_QTL_list_full %>% 
  distinct(Gene, .keep_all = TRUE) %>% 
  ##first add pvalue to rsid list
  left_join(., (SNP_supp %>% dplyr::select(RSID,Pvalue)),by = join_by(RSID)) %>%
  left_join(., list_snp_IDS, by = c("SYMBOL"="hgnc_symbol", "Gene"="ensembl_gene_id")) %>% 
            
            dplyr::select(SYMBOL,Gene,entrezgene_id,Pvalue) 

complete_list <- bind_rows(list(SNP_genes=SNP_genes,egene_of_SNP=egene_of_SNP),.id ="id" )
  ##got list of unique ESGNs to try to match up all entrezids
# ESGN_retrieve <- complete_list %>% 
#   mutate(ESGN= if_else(ensembl_gene_id=="NA",Gene,ensembl_gene_id)) %>% 
#   dplyr::select(ESGN) %>% 
#     distinct(ESGN)
    
# ESGN_rds <- getBM(attributes = my_attributes,
#                      filters ="ensembl_gene_id",
#                      values =ESGN_retrieve$ESGN,
#                      mart = ensembl)
# saveRDS(ESGN_rds,"output/ESGN_rds.RDS")
#   saved as an .RDS so I could match it up below
# complete_list <- 
  complete_list %>% 
  mutate(ESGN = coalesce(ensembl_gene_id,Gene)) %>% 
  dplyr::select(ESGN,entrezgene_id,SYMBOL,Pvalue,id) %>% 
    left_join(., ESGN_rds, by=c("ESGN"="ensembl_gene_id","SYMBOL"="hgnc_symbol")) %>% 
    mutate(ENTREZID=coalesce(entrezgene_id.y,entrezgene_id.x)) %>% 
    dplyr::select(ESGN,SYMBOL, Pvalue,id,ENTREZID) %>% 
    distinct() %>% 
    arrange(ENTREZID,by=(Pvalue)) 
  ##saved after all the arrangement above
...
SGWAS_sig_order <- Schneider_GWAS %>% 
  ...
  arrange(Pvalue) %>% 
  dplyr::select(!SYMBOL) %>% 
  left_join(.,backGL, by = c("ENTREZID"= "ENTREZID"))
  # dplyr::select(SYMBOL) #%>% 
  # distinct()
SGWAS_sig <- FC_mine %>% 
  dplyr::filter(ENTREZID %in% Schneider_GWAS$ENTREZID) %>% 
  mutate(drug=factor(id, levels = c('DOX','EPI','DNR','MTX','TRZ','VEH'))) %>%
 filter(time =="24_hours") %>% 
  dplyr::select(ENTREZID , id,logFC, adj.P.Val, SYMBOL) %>%
  pivot_wider(id_cols=id, 
...
ComplexHeatmap::Heatmap(SGWAS_mat, name = "Fold change\nvalues", 
        ...
        # col = col_funFC,
        row_order = c('DOX','EPI','DNR', 'MTX','TRZ'),
        column_title = "Full GWAS from Schneider",
        column_title_side = "top",
        column_title_gp = gpar(fontsize = 16, fontface = "bold"),
        # column_order = 
            column_names_rot = 90, 
...
logFtestSGWAS <- framefun24 %>% 
  mutate(DOX_plog=(log10(DOX.VEH.24)*(-1)),
         ...
         TRZ_plog=(log10(TRZ.VEH.24)*(-1)),) %>% 
  dplyr::select(ENTREZID,DOX_plog,EPI_plog, DNR_plog, MTX_plog,TRZ_plog) %>% 
  dplyr::filter(ENTREZID %in% SGWAS_sig_order$ENTREZID) %>% 
  pivot_longer(cols = !ENTREZID, names_to = "treatment", values_to = "plog") %>% 
  separate(treatment, into=c("treatment",NA)) %>% 
  mutate(ENTREZID=as.numeric(ENTREZID)) %>% 
  left_join(., (backGL)) %>% 
...
ComplexHeatmap::Heatmap(logFtestSGWAS, name = "-log10\np value", 
        ...
        # col = col_funFC,
        row_order = c('DOX','EPI','DNR', 'MTX','TRZ'),
        column_title = " Variance in Full GWAS from Schneider",
        column_title_side = "top",
        column_title_gp = gpar(fontsize = 16, fontface = "bold"),
        # column_order = 
            column_names_rot = 90, 
...
egenes_top50SNP <- SNP_egenes_allfiles%>% 
  dplyr::filter(RSID %in%SNP_top50$RSID) %>% 
  distinct(ensembl_gene_id,.keep_all = TRUE) %>% 
  left_join(., (SNP_supp %>% dplyr::select(RSID,Pvalue)),by = join_by(RSID)) %>% dplyr::select(SYMBOL, ensembl_gene_id, entrezgene_id,Pvalue)
###33 seperate egenes (pvalue is associated with the RSID and that gene)

SNP_genestop50 <- RSID_QTL_list_full %>% 
  distinct(Gene, .keep_all = TRUE) %>% 
  ##first add pvalue to rsid list
  dplyr::filter(RSID %in% SNP_top50$RSID) %>% 
  left_join(., (SNP_top50 %>% dplyr::select(RSID,Pvalue)),by = join_by(RSID)) %>%
  left_join(., list_snp_IDS, 
            by = c("SYMBOL"="hgnc_symbol", "Gene"="ensembl_gene_id")) %>% 
  dplyr::select(SYMBOL,Gene,entrezgene_id,Pvalue) %>% 
  rename("ensembl_gene_id"="Gene")
top50full <- bind_rows(list(SNP_genes=SNP_genestop50,egene_of_SNP=egenes_top50SNP),.id ="id" )
top50full %>% 
  dplyr::filter(entrezgene_id %in% backGL$ENTREZID) %>% 
  arrange(Pvalue)

SGWAS_top50_order <- top50full %>% 
  dplyr::filter(entrezgene_id %in%backGL$ENTREZID) %>% 
  arrange(Pvalue) %>% 
  dplyr::select(!SYMBOL) %>% 
...
SGWAS50_sig <- FC_mine %>% 
  dplyr::filter(ENTREZID %in% top50full$entrezgene_id) %>% 
  mutate(drug=factor(id, levels = c('DOX','EPI','DNR','MTX','TRZ','VEH'))) %>%
 filter(time =="24_hours") %>% 
  dplyr::select(ENTREZID , id,logFC, adj.P.Val, SYMBOL) %>%
  pivot_wider(id_cols=id, 
...
gene_corr_frame %>% 
  dplyr::select(indv,Drug,counts,tox,hgnc_symbol)
