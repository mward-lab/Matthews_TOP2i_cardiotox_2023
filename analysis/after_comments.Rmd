...
RSID_QTL_list_full <- read.csv("output/RSID_QTL_list_full.txt", row.names = 1)##closest annotated genetto complete rsid list
# Knowles_supp13 <- elife_33480_supp13
# write.csv(Knowles_supp13, "output/Knowles_supp13.csv")
...
SNP_gene_list <- near_genes_SNP1 %>% 
  ...
  full_join(.,SNP_supp,by="RSID") %>% 
  dplyr::select(RSID, SYMBOL,Gene,Ch,BP,OR,Pvalue) %>%
  left_join(.,list_snp_IDS,by=c("Gene"="ensembl_gene_id"))
