...
```{r retrieving TSSstartstop and IDs}
...
SNPXgenes <- just_the_SNPs %>% 
dplyr::filter(ENTREZID %in% backGL$ENTREZID) %>% 
  left_join(., (backGL %>% dplyr::select(ENTREZID,ensembl_gene_id,SYMBOL,hgnc_symbol)), by=c("ENTREZID"="ENTREZID","ensembl_gene_id"="ensembl_gene_id")) %>% 
  mutate(SYMBOL= if_else(is.na(SYMBOL),"NDN",SYMBOL)) %>% 
  dplyr::select(SYMBOL, ensembl_gene_id, ENTREZID,Pvalue) %>% 
...
ComplexHeatmap::Heatmap(SGWAS_mat, name = "Fold change\nvalues", 
        ...
        # col = col_funFC,
        row_order = c('DOX','EPI','DNR', 'MTX','TRZ'),
        column_title = "Full Schneider SNP-genes and eGenes",
        column_title_side = "top",
        column_title_gp = gpar(fontsize = 16, fontface = "bold"),
        # column_order = 
            column_names_rot = 90, 
...
schneider_closest_output <- readRDS("data/schneider_closest_output.RDS")
SNP_genestop50 <- schneider_closest_output %>% 
  arrange(., Pvalue) %>% 
  # dplyr::select(RSID,Pvalue,ENTREZID,ensembl_gene_id) %>% 
...
completetop50 <- Schneider_GWAS %>% 
  dplyr::filter(Pvalue <= max(SNP_genestop50$Pvalue))
 

SGWAS_top50_order <- completetop50 %>% 
  dplyr::filter(entrezgene_id %in%backGL$ENTREZID) %>% 
  arrange(Pvalue) %>% 
  dplyr::select(!SYMBOL) %>% 
  left_join(.,backGL, by = c("entrezgene_id"= "ENTREZID"))
...
SGWAS50_sig <- FC_mine %>% 
  dplyr::filter(ENTREZID %in% completetop50$entrezgene_id) %>% 
  mutate(drug=factor(id, levels = c('DOX','EPI','DNR','MTX','TRZ','VEH'))) %>%
 dplyr::filter(time =="24_hours") %>% 
  dplyr::select(ENTREZID , id,logFC, adj.P.Val, SYMBOL) %>%
  pivot_wider(id_cols=id, 
...
base_egeneset<- chisq.test(Knowsvar_base, correct = FALSE)$p.value
base_egeneset
...
resp_egeneset<- chisq.test(Knowsvar_respE, correct = FALSE)$p.value
resp_egeneset
...
Seoane_Var <-  matrix(c(13, 317, 354, 13400),
  ...
  byrow = FALSE,
  dimnames = list(c("Hvar", "nHvar"), c("y", "n")))
all_chrom_regs<- chisq.test(Seoane_Var, correct = FALSE)$p.value
all_chrom_regs

Seoane_Var2 <-  matrix(c(3, 47, 364, 13670),
  nrow = 2,
  ncol = 2,
  byrow = FALSE,
  dimnames = list(c("Hvar", "nHvar"), c("y", "n")))
AC_sens_chrom_regs<- chisq.test(Seoane_Var2, correct = FALSE)$p.value
AC_sens_chrom_regs
