...
readRDS("data/gene_corr_frame.RDS")
...
SL25_corr_frame <- gene_corr_df %>% 
  ...
  # storetest <- RARG_corr_frame %>% 
  # dplyr::select(indv,Drug,LD50,rtnni,rldh,Viability,counts) %>% 
  # dplyr::group_by(Drug) %>% 
  #   summarize(across(LD50:counts, ~ cor(.,counts)))
 rarg_plot_data <- ggplot_build(RARG_plotld50)
 rarg_data <- data.frame('rho'= rarg_plot_data$data[[3]]$r, 'sig'=c(rarg_plot_data$data[[3]]$p.value))
 
 stat_cor_extract(RARG_plotld50,'LD50','counts')
...
slc28A3_plot_data <- ggplot_build(slc_plotld50)
 slc28a3_data <- data.frame('rho'= slc28A3_plot_data$data[[3]]$r, 'sig'=slc28A3_plot_data$data[[3]]$p.value)
