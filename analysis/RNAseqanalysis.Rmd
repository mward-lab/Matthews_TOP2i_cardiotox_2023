...
```{r individual PCA by drug and time steps, echo=FALSE, warning=FALSE}
...
autoplot(saveddata, data= x=2,y=3)
...
library(ggrepel)
ggplot(pca_all_anno, aes(x = PC2, y = PC3))+
  geom_point( col=indv, size = time, label=group) +
  geom_text(aes(label=group))+
    labs(x = "PC2", y = "PC3")+
  theme_bw()+
  guides(shape=TRUE,color=TRUE)
