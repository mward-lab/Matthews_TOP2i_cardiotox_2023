...
###Color sets:   individuals: c(#7007071, #41B333,)
```
...
geom_boxplot(aes(fill=as.factor(indv)))+ 
  ...
  ylim(c(0,10))+
labs(x= "", fill ="Individual",y ="RNA Integrity Number")+ 
...
labs(x= "", fill ="Time in hours",y ="RNA Integrity Number")+ 
  ggtitle("Boxplot of RIN by time")
...
print(pca_all_anno)
pca_var_plot
store <- calc_pca(t(dat_cpm))### using the calc_pca function
store.var <- store$sdev^2
prop.store <- store.var/sum(store.var)

# prop.store %>%as.data.frame() %>%  
#   ggplot(., aes(x=1, y=2))+
  
saveRDS(pca_all_anno,"data/supp_pca_all_anno.RDS")
```
...
library(ggfortify)
# saveddata <- prcomp(dat_cpm)
# autoplot(saveddata,  x=1,y=2)
