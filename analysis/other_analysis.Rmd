...
erchi<-  chi_list %>% 
    summarise(pvalue= chisq.test(ER,chrom)$p.value) 
tichi <-  chi_list %>% 
    summarise(pvalue= chisq.test(TI,chrom)$p.value)
lrchi <-  chi_list %>% 
    summarise(pvalue= chisq.test(LR,chrom)$p.value)
nrchi <-  chi_list %>% 
    summarise(pvalue= chisq.test(NR,chrom)$p.value)  
Seoane_4suppchi=tibble(motif=c("ER","TI","LR","NR"),pvalue=c(erchi$pvalue,tichi$pvalue,lrchi$pvalue,nrchi$pvalue))
Seoane_4suppchi %>%
  mutate_at(.vars = 2, .funs= scientific_format()) %>% 
  kable(., caption= "P values from chisqure test" )%>% 
  kable_paper("striped", full_width = FALSE) %>%  
  kable_styling(full_width = FALSE, position = "left",bootstrap_options = c("striped"),font_size = 18) %>% 
  scroll_box(width = "60%", height = "400px")
...
nrchi <-  chi_list1 %>% 
    summarise(pvalue= chisq.test(NR,chrom)$p.value)  
Seoane_1suppchi=tibble(motif=c("ER","TI","LR","NR"),pvalue=c(erchi$pvalue,tichi$pvalue,lrchi$pvalue,nrchi$pvalue))
Seoane_1suppchi %>%
  mutate_at(.vars = 2, .funs= scientific_format()) %>% 
  kable(., caption= "P values from chisqure test" )%>% 
  kable_paper("striped", full_width = FALSE) %>%  
