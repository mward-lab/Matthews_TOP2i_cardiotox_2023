...
Knowles_count <- toplistall %>%
  ...
  ggplot(Knowles_count, aes(x=QTL_type,y=gene_count, group=group,  fill=group))+
   ...
   Knowles_count %>% 
     spread(gene_chout,group) #%>% 
  summarize(pvalue= chisq.test(gene_count,group)$p.value) 
     
    tab <- tapply(Knowles_count$count, list(good_example$category, good_example$variable), FUN=sum)
