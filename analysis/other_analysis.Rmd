...
```{r updated reQtl dox}
  ...
  tally() %>% 
  kable(., caption= "All genes (sig and nonsig) brokendown by drug and DOXreQTL status") %>% 
...
# dataframgtex %>% 
#   pivot_longer(., id_cols=c(id, time, sigcount), names_to="GTEx"
#   
# ggplot(dataframgtex, aes(x=QTL_type,y=gene_count, group=group,  fill=group))+
#     geom_col(position='fill')+
#     guides(fill="none")+
#     facet_wrap(id~time,nrow=2,ncol=4)+
#     theme_classic()+
