...
mean_lfc <- lfc_nums %>% 
  ...
  summarize(ERm=mean(absFC[ER=="y"]),TIm=mean(absFC[TI=="y"]),LRm=mean(absFC[LR=="y"]),NRm=mean(absFC[NR=="y"])) %>% as.data.frame()


mean_lfc %>% 
  pivot_longer(!c(id,time), names_to = "Motif",values_to="meanLFC") %>% 
  ggplot(., aes(x=time,y= meanLFC,col=Motif,group=Motif))+
  geom_point()+
  geom_line()+
  # fill_palette(palette =drug_palNoVeh)+
  # guides(fill=guide_legend(title = "Treatment"))+
