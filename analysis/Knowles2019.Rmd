...
Knowles_count <- 
  ...
  mutate(QTL_type =case_match(QTL_type, 'K4'~'base\negenes',
  ...
  mutate(group=factor(group, levels= c("n","y"), labels=c("not DE","DE")))
...
When I look ot the reQTLs across treatments at 24 hours, I see that dox has a total of `r length(DOXreQTLs$ENTREZID)` reQTLS.  Because this gene set was specifically about DOX response eQTLs, I wanted to see if these reQTLs were only Dox specific, or had more in common with other treatments.
...
```{r DoxreQTl over lap sigDEGS}
library(paletteer)
reQTL_overlapDE24 <- list(DOXreQTLs$ENTREZID,sigVDA24$ENTREZID, sigVEP24$ENTREZID, sigVMT24$ENTREZID, DoxonlyDEG)
re_incommon <- c(DOXreQTLs$ENTREZID,sigVDA24$ENTREZID, sigVEP24$ENTREZID, sigVMT24$ENTREZID, DoxonlyDEG)

names(reQTL_overlapDE24) <- c("Dox_reQTLS", "DNR DEGs","EPI DEGs","MTX DEGs", "DoxonlyDEG")
...
# gostres3Dnrdeg_sp <- gost(query = DNR3deg_sp$ENTREZID,
#                       organism = "hsapiens",
#                       ordered_query = FALSE,
#                       domain_scope = "custom",
#                       significant = FALSE,
...
```{r 3hour gene example}

Dox3genesp_example <- "none"#cpm_boxplot(cpmcounts,GOI=101927720,"Dark2",drug_palc,
  #               ylab=bquote(~italic("ZNF793-AS1 ")~log[2]~"cpm "))+ 
  # theme(axis.title = element_text(size = 10, color = "black"), axis.text = element_text(size = 10, color = "black", angle = 0))

Dnr3genesp_example <- cpm_boxplot_time(cpmcounts,'3h',51557 ,"Dark2",drug_palc,
                ylab=bquote(~italic("LGSN")~log[2]~"cpm "))+ 
   theme(axis.title = element_text(size = 10, color = "black"), axis.text = element_text(size = 10, color = "black", angle = 0))
...
Mtx3genesp_example <- cpm_boxplot(cpmcounts,GOI=284306,"Dark2",drug_palc,
...
#### grid 3 hour
```{r 3hour grid,eval=TRUE,echo=FALSE, fig.width=14, fig.height=8.5}
...
allfinal3hour <- plot_grid(Doxo3pg,Epi3pg,Daun3pg,mito3pg,nrow=4)
allfinal3hour
