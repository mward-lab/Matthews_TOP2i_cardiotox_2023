...
Epi3onlyDEG <- venn_3h$..values..[[12]]## 8 genes
...
Dox3Bplot <- toplist3hr %>% 
  ...
  geom_boxplot(aes(fill=id))+
  xlab(" ")+
  ylab("DOX specific adj.P.V <0.03\n n = 0")+
  theme_classic()+
  fill_palette(palette = drug_palc)+
  guides(fill= FALSE)+
  theme(
...
Dnr3genesp_example <- cpm_boxplot(cpmcounts,51557 ,"Dark2",drug_palc,
                ylab=bquote(~italic("LGSN")~log[2]~"cpm "))+ 
   theme(axis.title = element_text(size = 10, color = "black"), axis.text = element_text(size = 10, color = "black", angle = 0))


Epi3genesp_example <- cpm_boxplot(cpmcounts,GOI=79169,"Dark2",drug_palc,
