  ...
  ##order of dataframe should have time,id, ENTREZID,SYMBOL logFC,AveExpr,and adj.P.Val (8 columns)
    ...
    plot <- ggplot2::ggplot(df_plot, aes(x=drug, y=counts))+
      ...
      theme(strip.background = element_rect(fill = "white"),
          ...
          axis.text.x = element_text(size = 10, color = "white", angle = 0),
...
toplistall <- readRDS("data/toplistall_final.RDS")
...
intersect(DoxonlyDEG,EpionlyDEG)
...
DOXdeg_sp <- toplist24hr %>% 
  # dplyr::filter(ENTREZID %in% DoxonlyDEG) %>% 
  dplyr::filter(if_else(id=="Doxorubicin",adj.P.Val<0.01,adj.P.Val>0.01 )) #%>% 
...
```
##### Grid for supplement
```{r Agrid plot, eval=FALSE,echo=FALSE}
densityDOXsp <- toplist24hr %>% 
  filter(ENTREZID %in% DoxonlyDEG) %>% 
  ggplot(., aes(x=adj.P.Val))+
  geom_density(aes(fill=id, alpha= 0.8))+
...
```{r Bgrid plot, eval=FALSE,echo=FALSE, fig.width=14, fig.height=8.5}

 DoxBplot <- toplist24hr %>% group_by(time,id) %>% 
  dplyr::filter(ENTREZID %in% DoxonlyDEG) %>% 
  dplyr::filter(adj.P.Val <0.05) %>% 
  ggplot(., aes(x=adj.P.Val))+
  geom_histogram(aes(fill=id))+
...
Doxgenesp_example <- cpm_boxplot(cpmcounts,GOI=79065,"Dark2",drug_palc,
                ylab=bquote(~italic("ATG9A")~log[2]~"cpm "))

Dnrgenesp_example <- cpm_boxplot(cpmcounts,10081,"Dark2",drug_palc,
                ylab=bquote(~italic("PDCD7")~log[2]~"cpm "))


Epigenesp_example <- cpm_boxplot(cpmcounts,GOI=85457,"Dark2",drug_palc,
                ylab=bquote(~italic("CIPC")~log[2]~"cpm "))


Mtxgenesp_example <- cpm_boxplot(cpmcounts,GOI=26164,"Dark2",drug_palc,
                ylab=bquote(~italic("MTG2")~log[2]~"cpm "))



mitopg <- plot_grid(densityMTXsp, MtxBplot, Mtxgenesp_example, nrow = 1, rel_heights = c(.8,1.5,1), rel_widths=c(1,1.5,2),scale=c(1,1,0.8))


Daunpg <- plot_grid(densityDNRsp, DnrBplot, Dnrgenesp_example, nrow = 1, rel_heights = c(.8,2,1), rel_widths=c(1,1.5,2),scale=c(1,1,0.8))
Doxopg <- plot_grid(densityDOXsp, DoxBplot, Doxgenesp_example, nrow = 1, rel_heights = c(.8,2,1), rel_widths=c(1,1.5,2),scale=c(1,1,0.8))
Epipg <- plot_grid(densityEPIsp, EpiBplot, Epigenesp_example, nrow = 1, rel_heights = c(.8,1.5,1), rel_widths=c(1,1.5,2),scale=c(1,1,0.8))
allfinal <- plot_grid(Doxopg,Daunpg,Epipg,mitopg,nrow=4)
