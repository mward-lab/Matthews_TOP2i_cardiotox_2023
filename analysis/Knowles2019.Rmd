...
reQTLcombine=list(DOXreQTLs$ENTREZID,EPIreQTLs$ENTREZID,DNRreQTLs$ENTREZID,MTXreQTLs$ENTREZID)
...
DoxonlyQTL <-  QTLoverlaps$..values..[[14]]
     ggVennDiagram::ggVennDiagram(reQTLcombine, category.names = c("DOX-142 \nsigDEG\n","EPI-131\nsigDEG\n","DNR-148\nsigDEG","MTX-27\nsigDEG"),label = "count") +scale_x_continuous(expand = expansion(mult = .2))+scale_y_continuous(expand = expansion(mult = .1))+scale_fill_distiller(palette="RdYlGn", direction = -1)
```
...
```{r a little 24 GO analysis, eval=TRUE}
library(gprofiler2)
...
```{r Bgrid plot, eval=TRUE,echo=FALSE, fig.width=12, fig.height=8.5}
...
```{r a little 3 hGO analysis, eval=TRUE}
...
```{r mtx specific enrichment plots, fig.height=10, fig.width=6}
Mtx3_sp_DEGtable  %>% 
    dplyr::filter(source=="GO:BP") %>% 
    dplyr::select(p_value,term_name,intersection_size) %>%
    slice_min(., n=10 ,order_by=p_value) %>%
