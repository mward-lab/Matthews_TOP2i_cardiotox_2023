...
ggplot(avg_slope, aes(x=rowname, y=name))+
  geom_boxplot(position="dodge", aes(fill=rowname))+
    ...
    theme(axis.title=element_text(size=10),
          ...
          plot.title = element_text(hjust = 0.5, size =15, face = "bold"))
```
```{r slope correlation, eval=FALSE, message=FALSE, warning=FALSE, include=FALSE}
 read.csv("data/slope_table.csv",row.names = 1)
avgSlope <- slope_table %>% 
  rownames_to_column() %>% 
  as.tibble()%>% 
  mutate(rowname=case_match(rowname,'1'~'DNR','2'~'DOX','3'~'EPI','4'~'MTX', .default = rowname))%>% 
  pivot_longer(!rowname,names_to = 'indv', values_to = 'slope') %>% 
  ...
  group_by(rowname,indv) %>% 
  summarise_at(vars(slope),list(name = mean)) %>% 
  rename('Treatment'='rowname',"slope"="name")

avgLD50 <- readRDS("data/avgLD50.RDS")

slope_ld50 <- avgLD50 %>%  
  mutate(Treatment= case_match (Treatment, 'Daun'~'DNR','Doxo'~'DOX','Epi'~'EPI','Mito'~'MTX', .default = Treatment)) %>% 
  right_join(., avgSlope, by=c('Treatment','indv'))

slope_ld50 %>% 
  ggplot(., aes(x=slope, y=LD50))+
  ...
  ggtitle(expression("Correlation between LD"[50]*" and slope"))
```
