...
###making a dataframe to transform the individuals into a long large data frame##3
 ...
 smaller2 <- individuals[c(1,3,5,7,9,11)]
...
```{r Viablitiy plots, warning=FALSE}
### to extract parameters for mapping
DAvia <- full_list %>% filter(Drug == "Daun") %>% filter(Conc <= 1) %>% mutate(SampleID=substr(SampleID,1,4))
DXvia <- full_list %>% filter(Drug == "Doxo")%>% filter(Conc <= 1) %>% mutate(SampleID=substr(SampleID,1,4))
EPvia <- full_list %>% filter(Drug =="Epi")%>% filter(Conc <= 1) %>% mutate(SampleID=substr(SampleID,1,4))
MTvia <- full_list %>% filter(Drug =="Mito")%>% filter(Conc <= 1) %>% mutate(SampleID=substr(SampleID,1,4))
TRvia <- full_list %>% filter(Drug =="Tras")%>% filter(Conc <= 1) %>% mutate(SampleID=substr(SampleID,1,4))
VEvia <- full_list %>% filter(Drug == "Veh")%>% filter(Conc <= 1) %>% mutate(SampleID=substr(SampleID,1,4))
alterlist <- full_list %>% na.omit()#%>% filter(Conc <= 1) %>% mutate(SampleID=substr(SampleID,1,4)) 

ggplot(alterlist, aes(x= as.factor(Conc), y= Percent, group =Conc,color = SampleID, alpha =0.8)) +
     guides(alpha = "none")+ 
      # stat_summary(fun.data=mean_se, fun.args = list(mult=1), geom = "errorbar", size =1, width = 1)+
  geom_violin(position ="dodge",outlier.colour="transparent")+
      ylim(0,1.5)+
        # Change the x-axis scale to log 10 scale
  theme_classic() +
  # scale_x_discrete("Conc", labels = c("0.01","0.05", "0.1","0.5","1"))+
  xlab(expression(paste("Concentration [", mu, "M]")))+
  geom_hline(yintercept = 1,lty = 3)+
  ylab(NULL) +
  facet_wrap(~Name)+
  theme(plot.title = element_text(hjust = 0.5, size =15, face = "bold"))+
      ggtitle("Viablity at low concentrations")+
...
ggplot(DA2, aes(x=Conc, y= Percent, color = SampleID, alpha =0.6)) +
  ...
  geom_function(fun=mean_se)+
    stat_smooth(method = "mean_se")+
      ylim(0,1.5)+
...
daplot <- 
  ggplot(DA2, aes(x=Conc, y= Percent, color = SampleID,alpha =0.4)) +
     guides(color="none", alpha = "none")+
      stat_summary(fun.data=mean_se, fun.args = list(mult=1), geom = "errorbar", size =.5, width = .1)+
...
trplot <- 
  ...
  l# theme(plot.title = element_text(hjust = 0.5, size =20))
  trplot  
...
LDfull <- rbind(daunls,doxols,epils,mitols)
rownames(LDfull) <- c("Daunorubicin", "Doxorubicin", "Epirubicin", "Mitoxantrone")
summary(LDfull)
LD50long <- pivot_longer(data = LDfull,cols = names_to ="Individual", values_to= "ld50")

LD50long <- LDfull %>% tibble::rownames_to_column(var = "Treatment") %>% pivot_longer(-Treatment, cols_vary = "slowest", names_to="indv",values_to="ld50")
# LD50full <-
...
ab <- LD50long %>% 
  ggplot(., mapping = (aes(x = (Treatment), y = log10(ld50)))) +
  geom_point(aes(
    color = indv,
    size = 5,
    shape = indv,
...
ldall <- bind_rows(daunls,doxols, epils, mitols)
str(ldall
  )
