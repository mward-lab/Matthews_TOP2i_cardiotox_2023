...
full_list <- read.csv("output/output-old/LDH48hoursdata.csv")
...
summary(LDfull)
BC_cell_lines <- read_excel("~/Ward Lab/Cardiotoxicity/Kandace Planning LDH/BC_cell_lines.xlsx", 
+     sheet = "BC_sample_set")
...
avgLD50 %>% mutate(Treatment=case_match(Treatment,"Daun"~"Daunorubicin",
  ...
  geom_point(aes(color = indv,
                 size = 5,alpha = 0.5)) +
  ggtitle(expression("Observed LD"[50]*"s"))+
  xlab("Treatment")+
...
BC_cell_lines %>%
  ...
  geom_point(aes(color = Cell_line,
                 size = 5,alpha = 0.5)) +
  ggtitle(expression("Cell line reported  LD"[50]*"s"))+
  xlab("Treatment")+
  ylab(bquote('Log'[10]~ 'LD'[50]~'in '*mu*M))+
