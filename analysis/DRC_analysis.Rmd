...
ggplot(VE, aes(x= as.factor(Conc), y= Percent)) +
  ...
  facet_wrap(~SampleID)
    ...
    ```{r  Viability data , warning=FALSE}
### to extract parameters for mapping
...
ggplot(alterlist, 
       aes(x= as.factor(Conc), y= Percent, group =Conc,
           color = SampleID, alpha =0.5)) +
  guides(alpha = "none")+
  geom_point()+
...
meanLDfull %>% pivot_wider(.,id_cols=Treatment)
...
LD50long <- LDfull %>% tibble::rownames_to_column(var = "Treatment") %>% pivot_longer(-Treatment, cols_vary = "slowest", names_to="indv",values_to="ld50") %>%  mutate(indv=substr(indv,1,4)) %>% group_by(Treatment,indv) %>% mutate(mean =mean(ld50)) %>% summarise()
