...
ggplot(avg_slope, aes(x=rowname, y=name))+
  geom_boxplot(position="dodge", aes(fill=rowname))+
    ...
    theme(axis.title=element_text(size=10),
          ...
          plot.title = element_text(hjust = 0.5, size =15, face = "bold"))
```
```{r slope correlation, fig.keep='all',eval=FALSE, message=FALSE, warning=FALSE, include=FALSE}
  slope_table <- read.csv("data/slope_table.csv",row.names = 1)

slopecorr <- cor(slope_table)
heatmap(slopecorr,cluster_rows=FALSE,cluster_cols = FALSE, main="slope")
...
Heatmap(slopecorr, cluster_rows = FALSE,cluster_columns = FALSE, col = col_fun1,column_title = expression("Slope correlation"),row_title=" ",row_split = factor(rep(c("1","2","3","4","5","6"),each=2)), column_split = 
    factor(rep(c("1","2","3","4","5","6"),each=2)))

Heatmap(slopecorr, cluster_rows = FALSE,cluster_columns = FALSE, column_title = expression("Slope correlation"),row_title=" ",row_split = factor(rep(c("1","2","3","4","5","6"),each=2)), column_split = 
    factor(rep(c("1","2","3","4","5","6"),each=2)))
