...
individuals <- list(ind1a,  ###compile the separate data frames into a list of data frames
                    ind1b,
                    ind2a,
                    ind2b,
                    ind3a,
...
names(individuals) <- c("ind1a",####adds the names to dataframe list
                          'ind1b',
                          'ind2a',
                          'ind2b',
                          "ind3a",
                          ...
                          "ind6a",
                          "ind6b")
ID <- c("ind1a",  ###creates a list called ID which holds the names if needed
        'ind1b',
        'ind2a',
        'ind2b',
        "ind3a",
...
samller <- bind_rows(smaller)
samller2 <- bind_rows(smaller2)

```

I need to creat columns called
...
unique(samller$SampleID)
#returns "ind1a" "ind2b" "ind3b" "ind4b" "ind5a" "ind6a"
DA <- abc_df %>% filter(Drug == "Daun")
DX <- abc_df %>% filter(Drug == "Doxo")
EP <- abc_df %>% filter(Drug =="Epi")
MT <- abc_df %>% filter(Drug =="Mito")
TR <- abc_df %>% filter(Drug =="Tras")
VE <- abc_df %>% filter(Drug == "Veh")
DA2 <- samller2 %>% filter(Drug == "Daun")
DX2 <- samller2 %>% filter(Drug == "Doxo")
EP2 <- samller2 %>% filter(Drug =="Epi")
MT2 <- samller2 %>% filter(Drug =="Mito")
TR2 <- samller2 %>% filter(Drug =="Tras")
VE2 <- samller2 %>% filter(Drug == "Veh")
#####Then add names!
```
...
mtplot <- 
  ggplot(MT2, aes(x=Conc, y= Percent, color = SampleID, alpha=0.6))+
      guides(color =  "none", alpha = "none")+
      stat_summary(fun.data=mean_se, fun.args = list(mult=1), geom = "pointrange", size =0.5)+
    stat_smooth(method = "drm", method.args = list(fct = L.4(c(NA,1,NA,NA))), se = FALSE)+
      ylim(0,1.5)+
      scale_x_log10() +  # Change the x-axis scale to log 10 scale
  theme_classic() +
   xlab(expression(paste("Concentration [", mu, "M]")))+
