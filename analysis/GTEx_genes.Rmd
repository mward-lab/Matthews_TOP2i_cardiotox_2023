...
GTEx_over <- intersect(GTEx_genes$entrezgene_id,my_exp_genes$ENTREZID)
nQTL <- not_eqtls
...
To find out how many genes from the gtex egenes were expressed in my data, I intersected my expressed genes list of `r length(my_exp_genes$ENTREZID)` genes with the GTEx_genes and found `r length(GTEx_over)` genes were shared between them. 
...
GTEX_table <-  toplistall %>%
  ...
  select( id, time,ENTREZID,GTEx,nQTL) %>% 
...
Interestingly, NONE of the genes show overlap
