...
intersect(DOX_reqtls, burr_genes$entrezgene_id)
# DOX_reqtls <-$ENTREZID %>% as.integer()
  DOXreQTLs %>% filter(ENTREZID %in% storeEPI$ENTREZID) %>%  tally( )
 burr_genes %>% 
   filter(entrezgene_id%in% backGL$ENTREZID) %>% 
   filter(entrezgene_id %in% storeEPI$ENTREZID)
