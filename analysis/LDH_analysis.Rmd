...
tnni24hstat
library(Biobase)
apple.pv <- t(as.data.frame(subListExtract(tnni24hstat,'p.value')))##this pulls out a specific thing and puts into datafram as a vector for later manipulation (think pvalue or mean)
# lapply(apple, function(x)
#   sapply(1:length(x),
#          function(y)do.call("[[",list(x,y))))
colnames(apple.pv) <- 'p.value'
