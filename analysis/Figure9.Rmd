...
col_funTOX = circlize::colorRamp2(c(-1,0, 1), c("darkgreen", "white","deeppink3"))
...
```{r ctxscore, message=FALSE, warning=FALSE, include=FALSE}

gene_corr_fig9 <- readRDS("output/gene_corr_fig9.RDS")
toxlist <- data.frame(ENTREZID=c(5916,23371,283337))
...
RARG_sig_mat3ptr <- toxdata3ptr %>% 
  dplyr::filter(gene=="RARG") %>% 
  dplyr::select(sig_tox) %>% 
  t() %>% 
  matrix()
RARG_mat3ptr <- toxdata3ptr%>% 
  dplyr::filter(gene=="RARG") %>% 
  dplyr::select(tox_val) %>% 
    t() %>% 
  matrix()
...
Heatmap(RARG_mat3ptr, name = " RARG 3pt Cardiotox score", 
         cluster_rows = FALSE,
        cluster_columns = FALSE, 
        row_names_side = "left",
        column_title_side = "top",
...
```{r RARG corrdata, eval=FALSE, fig.height=4, fig.width=8, include=FALSE}
gene_corr_fig9 <- readRDS("output/gene_corr_fig9.RDS")
RARG_data <- pearson_extract(gene_corr_fig9,5916)

rarg_mat <- RARG_data %>% 
  column_to_rownames('rowname') %>% 
   dplyr::select(rho_tnni,rho_ldh,rho_LD50) %>% 
  t() %>% 
  as.matrix()

rarg_mat_sig <- RARG_data %>% 
  column_to_rownames('rowname') %>% 
  dplyr::select(sig_tnni,sig_ldh,sig_LD50) %>% 
  mutate_all(~replace(., is.na(.), 1)) %>% 
  t() %>% 
  as.matrix()
col_funfinal= circlize::colorRamp2(c(-1,0, 1), c("#b35806","white", "#542788"))
...
```{r SLC data, eval=FALSE, fig.height=3, fig.width=4, include=FALSE}
...
```
```{r SCL cor data, eval=FALSE, fig.height=4, fig.width=8, include=FALSE}
...
```{r TNS2 cor data, eval=FALSE, fig.height=4, fig.width=8, include=FALSE}
...
```{r ZNF cor data, eval=FALSE, fig.height=4, fig.width=8, include=FALSE}
