...
```{r expression correlation, eval=TRUE, include=FALSE}
...
# now I have the long dataframe, time to combine with other data 
  gene_corr_frame <- gene_corr_df %>% 
    filter(entrezgene_id==GOI_genelist$entrezgene_id) %>% 
left_join(., GOI_join, by=c("Drug","indv")) %>% 
  select(indv, Drug,entrezgene_id,counts, rldh,rtnni) %>% 
    mutate(Drug=factor(Drug))
  
 # gene_corr_frame %>% 
 #   group_by(entrezgene_id, rldh) %>% 

   
   library(Hmisc)
  rcorr(as.matrix(gene_corr_frame[,c(1,3,4,5,6)]), type = "pearson")
   
gene_corr_frame %>% 
  ggplot(., aes(x=rtnni, y=counts))+
  geom_point()+
  geom_smooth(method="lm")+
  facet_wrap(~indv, scales = "free")+
  theme_bw()+
...
gene_corr_frame %>% 
  ...
  ggtitle("Correlation between counts and troponin I")+
  ...
  stat_cor(method="pearson",aes(label = paste(..rr.label.., ..p.label.., sep = "~`,`~")),
           color = "red",
           label.x.npc = 0.01, label.y.npc=0.01, size = 3)+
...
gene_corr_frame %>% 
  ...
  xlab("troponin I expression") +
  ylab("Gene counts in log2 cpm") +
  ggtitle("Correlation by individual using counts and troponin I")+
  # scale_color_brewer(palette = "Dark2",name = "Individual", label = c("1","2","3","4","5","6"))+
...
gene_corr_frame %>% 
  ...
  xlab("troponin I expression") +
  ylab("Gene counts in log2 cpm") +
  ggtitle("Correlation between counts and 24 hour lactate dehydrogenase")+
  scale_color_brewer(palette = "Dark2",name = "Individual", label = c("1","2","3","4","5","6"))+
  stat_cor(method="pearson",aes(label = paste(..rr.label.., ..p.label.., sep = "~`,`~")),
           color = "red",
           label.x.npc = 0.01, label.y.npc=0.01, size = 3)+
...
gene_corr_frame %>% 
  ...
  xlab("troponin I expression") +
  ylab("Gene counts in log2 cpm") +
  ggtitle("Correlation between counts and 24 hour lactate dh")+
   # scale_color_brewer(palette = "Dark2",name = "Individual", label = c("1","2","3","4","5","6"))+
  scale_color_manual(values=drug_pal_fact)+
  stat_cor(method="pearson",aes(label = paste(..rr.label.., ..p.label.., sep = "~`,`~")),
           color = "red",
