...
```
Interestingly, Over half of the Daunorubicin specific genes are in the Daunorubicin 24 hour group.
...
All `r length(Daun3sp)` 3 hour Daun-specific genes are in the intersection of the daunorubicin sigDEGs for 3 hours and 24 hours. This could support the idea that DNR's more toxic effect is related to earlier impacts on cell function.  Note the enriched go terms for the 304 genes that they share.  I need to see if there is a trend in EP and DX.  I do see in Mito 3 hour (looked a little later).
...
ggVennDiagram::ggVennDiagram(list(sigVDA3$ENTREZID, sigVDA24$ENTREZID),
  ...
  scale_x_continuous(expand = expansion(mult = .2))+
  scale_fill_gradient(low = "red4", high = "tan1")+
  labs(title = "DEG p < 0.05, All daunorubicin 3hour and 24 hours sets", 
...
#     scale_y_discrete(labels = wrap_format(30))+
#     guides(col="none", size=guide_legend(title = "# of intersected \n terms"))+
#     ggtitle("Epirubicin specific 3 hour top KEGG terms") +
#      xlab(expression(" -"~log[10]~("adj. p-value")))+
#     ylab("KEGG term")+
#       theme_bw()+
#     theme(plot.title = element_text(size = rel(1.5), hjust = 0.5),
...
MTtable1 %>% 
 dplyr::filter(source=="GO:BP") %>% 
   ...
   ggplot(., aes(x = log_val, y =reorder(term_name,p_value), col= intersection_size)) +
    ...
    scale_y_discrete(labels = wrap_format(30))+
    guides(col="none", size=guide_legend(title = "# of intersected \n terms"))+
    ggtitle('Mitoxantrone 3 hour GO:BP terms') +
    xlab(expression(" -"~log[10]~("adj. p-value")))+
    ylab("GO: BP term")+
      theme_bw()+
    theme(plot.title = element_text(size = rel(1.5), hjust = 0.5),
...
DDErespAll <- (union(DDEresp,DDEMresp))
# DDEgostres24 <- gost(query = DDErespAll,
#                      organism = "hsapiens",
#                      ordered_query = TRUE,
#                      domain_scope = "custom",
#                      measure_underrepresentation = FALSE,
...
DDEtable24kegg <- DDEgostres24$result %>% 
  dplyr::select(c(source, term_id, term_name,intersection_size, term_size, p_value)) %>% 
  dplyr::filter(source=="KEGG")
...
## DDEM response of `r length(DDEMresp)`
```{r GO DDEM list of SDEGs, echo=FALSE, message=FALSE, warning=FALSE}
...
DXsptable24 %>% dplyr::filter(source=="GO:BP") %>% 
   ...
   ggplot(., aes(x = log_val, y =reorder(term_name,p_value), col= intersection_size)) +
    ...
    guides(col="none", size=guide_legend(title = "# of intersected \n terms"))+
    ggtitle(paste0("Doxorubicin specific DEGs (n = ",length(DXsprespon),") 24 hr GO:BP terms")) +
   xlab(expression(" -"~log[10]~("adj. p-value")))+
    ylab("GO: BP term")+
      theme_bw()+
    theme(plot.title = element_text(size = rel(1.2), hjust = 0.5),
...
ggplot(., aes(x = log_val, y=reorder(term_name, order_val, desc=TRUE),col=deg)) +
 ...
 facet_deg=(
   c(DAdeg = expression(atop("Daunorubicin 24 hr",~italic("n = 7017"))),
     DXdeg = expression(atop("Doxorubicin 24 hr",~italic("n = 6645"))),
     DXsp24deg =expression(atop("Dox-specific 24 hr",~italic("n = 355"))),
     EPdeg = expression(atop("Epirubicin 24 hr",~italic("n = 6328"))),
     MTdeg = expression(atop( "Mitoxantrone 24 hr",~italic("n = 1115"))),
     MTsp24deg =expression(atop( "Mito-specific 24 hr",~italic("n = 122"))),
     DDEdeg = expression(atop("Anthracycline 24 hr",~italic("n = 4435,"))),
...
note:  grey box means these terms were not fount to be enriched in the data set terms.  I will try to change the levels to pull out the pvalue of the term for each (6-30-23)
