...
Adding KAT6B and KDM5b and KDM4b(23030)
...
### KAT6b
cpmcounts %>% dplyr::filter(rownames(.)== 23522, ) %>%
  pivot_longer(everything(), names_to = "treatment",values_to = "counts") %>%
...
### KDM5b
cpmcounts %>% dplyr::filter(rownames(.)== 10765, ) %>%
  pivot_longer(everything(), names_to = "treatment",values_to = "counts") %>%
  ...
  facet_wrap("time",  nrow=1, ncol=2)+
  theme_bw()+
  ylab(expression(atop(" ",italic("KDM5"*beta)~log[2]~"cpm ")))+
  xlab("")+
  xlab("")+
  theme(strip.background = element_rect(fill = "#C77CFF"),
        plot.title = element_text(size=18,hjust = 0.5),
...
cpmcounts %>% dplyr::filter(rownames(.)== 23030, ) %>%
  pivot_longer(everything(), names_to = "treatment",values_to = "counts") %>%
...
GOIcorr <-(genes = c("RARG", "ZNF740", "KAZN", "SLC28A3", "FRS2" ,"HDDC2","UTGT1A6","UGT1A10","KDM4B","KAT6B","GNT1",'SLCO1A2', 'SLCO1B3'))
