...
DDEresp<- read.csv("data/DDEresp_list.csv")
...
kegg_mat<- keggtable%>% 
  ...
  pivot_wider(everything(),names_from="term_name",values_from="log_val") %>% 
 column_to_rownames('deg') %>% 
  as.matrix()# 

Heatmap(kegg_mat, name = "KEGG Pathway -log p values", cluster_rows = FALSE)
