...
scale_fill_distiller(palette="Spectral", direction = -1)+
  ...
  labs(title = "24 hour comparison of DE genes p.adj. <0.05", caption = "n = 8188 genes")+
...
DDEp24 <- gostplot(DDEgostres24only, capped = FALSE, interactive = TRUE)
DDEp24
...
DDEtable24kegg <- DDE24kegg$result %>% 
  dplyr::select(c(source, term_id, term_name,intersection_size, term_size, p_value)) 
DDEtable24<- DDEgostres24only$result %>% 
  dplyr::select(c(source, term_id, term_name,intersection_size, term_size, p_value)) 
...
cpmcounts %>% dplyr::filter(rownames(.)==5916) %>%
  pivot_longer(everything(), names_to = "treatment",values_to = "counts") %>%
...
# GOIcorr <-(genes = c("RARG", "ZNF740", "KAZN", "SLC28A3", "FRS2" ,"HDDC2" ))
...
# gene_corr_frame %>% 
#   filter(entrezgene_id ==10818) #%>% 
# ggplot(., aes(x=rtnni, y=counts))+
#   geom_point()+
#   geom_smooth(method="lm")+
#   facet_wrap(~Drug, scales = "free")+
#   theme_bw()+
#   stat_cor(aes(label = after_stat(paste(rr.label,p.label))),
#            color = "red")
## ggplot by drug for tnni
  ...
  for (gene in GOI_genelist$entrezgene_id){
    gene_plot <- gene_corr_frame %>% 
      ...
      geom_smooth(method="lm")+
      facet_wrap(entrezgene_id~Drug, scales="free")+
      theme_classic()+
...
## ggplot by indiv for tnni
 ...
 for (gene in GOI_genelist$entrezgene_id){
    gene_plot <- gene_corr_frame %>% 
      ...
      geom_smooth(method="lm")+
      facet_wrap(entrezgene_id~indv, scales="free")+
      theme_classic()+
      xlab("troponin I expression") +
      ylab("Gene counts in log2 cpm") +
...
kegg_mat<- keggtable%>% 
  ...
  pivot_wider(id_cols = everything(),names_from="term_name",values_from="log_val") %>% 
 column_to_rownames('deg') %>% 
  as.matrix()# 

Heatmap(kegg_mat, name = "KEGG Pathway -log p values", cluster_rows = FALSE, cluster_columns = FALSE)
